...
const dadosFormatados = dados.replace(/\n/g, ',');
// console.log(dadosFormatados);

const arrayDados = dadosFormatados.split(',');
// console.log(arrayDados);

function somaPrimeiroUltimo(string) {
    let primeiro, ultimo;

    for (let index = 0; index < string.length; index++) {
        if (!isNaN(parseInt(string[index]))) {
            primeiro = string[index];
            break;
        }
    }
...
const arraySomas = arrayDados.map((element) => somaPrimeiroUltimo(element));
console.log(arraySomas);
//console.log(arraySomas.filter((element) => isNaN(element)))

const soma = arraySomas.reduce((acc, number) => acc + parseInt(number), 0);
console.log(soma);